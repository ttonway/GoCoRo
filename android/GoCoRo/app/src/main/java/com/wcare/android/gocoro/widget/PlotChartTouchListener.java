 ...
 */
public class PlotChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<?
        extends IBarLineScatterCandleBubbleDataSet<? extends Entry>>>> {
    private static final String TAG = PlotChartTouchListener.class.getSimpleName();
    ...
    private float mMaxHighlightDistance;
     ...
     */
    public PlotChartTouchListener(BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends
        ...
        mMaxHighlightDistance = Utils.convertDpToPixel(40f);
    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchMode == DRAG) {
                    ...
                    if (mChart.hasNoDragOffset()) {
                        ...
                        if (mHighlight == null && !mChart.isFullyZoomedOut() && mChart.isDragEnabled()) {
                            mTouchMode = DRAG;
                        } else {
    ...
    private void saveTouchStart(MotionEvent event) {
        ...
        if (highlight != null) {
            float distance = getDistance(event.getX(), event.getY(), highlight.getXPx(), highlight.getYPx());
            if (distance < mMaxHighlightDistance) {
                mHighlight = highlight;
            }
        }
        Log.d(TAG, "saveTouchStart " + mHighlight + " === x: " + event.getX() + ", y: " + event.getY());
    }
